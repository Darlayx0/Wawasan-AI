...
const App = () => {
  ...
  // 3-State Toggle Cycle: Light -> Dark -> Sepia -> Light
  const toggleTheme = () => {
      setTheme(prev => {
    ...
    case AppState.READING:
      return (
         <ReadingView 
            ...
            onExplore={() => app.generateFeed(app.article!.topic)}
            onStartTool={(t) => app.startTool(t, false)} 
            onSelectTopic={(t) => { app.setTopic(t); app.navigate(AppState.HOME); }} 
    ...
    case AppState.TOOL_VIEW:
        return app.smartToolData ? 
            <SmartToolsView 
                ...
                onRegenerate={() => app.startTool(app.activeToolType!, true)} 
                theme={theme} 
            /> : null;
